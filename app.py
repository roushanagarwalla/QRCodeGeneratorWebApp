...
def generate_qrcode(text):
    ...
    if session['filetype'] == 1:
        qr.png(f'./static/temp/{filename}', scale=8, background=ImageColor.getcolor(session['background_hex'], "RGB"), module_color=ImageColor.getcolor(session['foreground_hex'], "RGB"))
    elif session['filetype'] == 2:
        qr.svg(f'./static/temp/{filename}', scale=8, background=session['background_hex'] ,module_color=session['foreground_hex'])

def set_filename(text):
    text = text + session['background_hex'] + session['foreground_hex'] + str(session['filetype'])
